(function($){
	...
	var NICK_MAX_LENGTH = 15,
		...
		tmplt = {
			...
			message: [
				'<div class="message">',
					'<span class="sender">${sender}: </span><p class="text">${text}</p><p class="time">${time}</p>',
				'</div>'
			].join("")
		};
	...
	function bindSocketEvents(){
		...
		socket.on('ready', function(data){
			...
			$('.chat-loading').animate({ 'opacity': 0 }, 200, function(){
				$(this).hide();
				$('#onChat').show();
				$('.chat input').focus();
			});
	...
	function createRoom(){
		...
		if(room && room.length <= ROOM_MAX_LENGTH && room != currentRoom){
			...
			$('.chat-loading').show().html('Creating room: ' + room + '...');
			$('.chat-loading').animate({ 'opacity': 1 }, 200);
